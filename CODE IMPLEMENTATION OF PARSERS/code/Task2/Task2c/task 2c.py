...
def grammarAugmentation(rules, nonterm_userdef,start_symbol):
	...
	# AUGMENTED GRAMMAR -> create unique 'symbol' to represent new start symbol
	newChar = start_symbol + "'"

	
	#if s' is in prodcution then make starting symbol as s''
	while (newChar in nonterm_userdef): #jb tk jo newchar h wo NTs me h
		newChar += "'"
...
def findClosure(input_state, dotSymbol):
	...
	#S' -> S. , ADD S wali all the prod in closure set
...
def createParseTableLR0aa(statesDict, stateMap, T, NT):
            ...
            if rule[1][-1] == '.':
                ...
                for key in numbered:
                    if numbered[key] == temp2:
                        # Put Rn in all columns for LR(0)
                        for col in cols:
...
createParseTableLR0aa(statesDict, stateMap,term_userdef,nonterm_userdef)
